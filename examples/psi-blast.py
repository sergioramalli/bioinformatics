...
try:
	...
	if from_existing_db != True:
		...
		fasta = blast.make_db(config['subject_db'], config['subject_sequences']).run(config['query_sequences'], config['psi_results'])

	else:

		# runs psiblast operation, and returns name of fasta file with Hits plus query 
		fasta = blast.set_db(config['subject_db'], config['subject_sequences']).run(config['query_sequences'], config['psi_results'])
